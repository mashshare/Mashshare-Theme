...
function eddpdfi_pdf_template_colours( $eddpdfi_pdf, $eddpdfi_payment, $eddpdfi_payment_meta, $eddpdfi_buyer_info, $eddpdfi_payment_gateway, $eddpdfi_payment_method, $address_line_2_line_height, $company_name, $eddpdfi_payment_date, $eddpdfi_payment_status ) {
	...
	$eddpdfi_pdf->SetFont( $font, '', 10 );
		if(!empty($eddpdfi_buyer_info['company'])){
			$eddpdfi_pdf->Cell( 0, 6, $eddpdfi_buyer_info['company'], 0, 2, 'L', false );
		}else {	
...
$eddpdfi_pdf->Cell( 0, 6, $country, 0, 2, 'L', false );
	...
	if ( $eddpdfi_pdf_downloads ) {
		...
		foreach ( $eddpdfi_pdf_downloads as $key => $download ) {
			...
			$eddpdfi_final_download_price = isset( $download['subtotal'] ) ? $download['subtotal'] : null;
	
			if( is_null( $eddpdfi_final_download_price ) ) {

				$eddpdfi_final_download_price = isset( $download['price'] ) ? $download['price'] : null;
			...
			//	$eddpdfi_download_price = iconv('UTF-8', 'windows-1252', $eddpdfi_download_price );
			//	$eddpdfi_total_price    = iconv('UTF-8', 'windows-1252', $eddpdfi_total_price );
			}
			...
			if ( eddpdfi_item_quantities_enabled() ) {
				...
				$eddpdfi_pdf->Cell( 20, $linecount * 5, $download['quantity'], 'LB', 0, 'C', false );
		...
		if ( function_exists( 'iconv' ) ) {
			// Ensure characters like euro; are properly converted. See GithuB issue #472 and #1570
		//	$subtotal = iconv('UTF-8', 'windows-1252', $subtotal );
		//	$tax      = iconv('UTF-8', 'windows-1252', $tax );
		}
		...
		if( function_exists( 'iconv' ) ) {
			// Ensure characters like euro; are properly converted. See GithuB issue #472 and #1570
		//	$total = iconv('UTF-8', 'windows-1252', $total );
		}
